...
powershell -ExecutionPolicy Bypass -File .\tools\flash-custom.ps1 `
  -Port COM3 -Version 1.3.5 -ConfirmCustomOnly ERASE-ORIGINAL
```
...
`ir-tracker-custom-1.3.5.irfw` laden. Signatur und ESP32-Image werden vor
...
`ir-tracker-assets-1.3.5.bin` installieren. Einstellungen, Historie und
...
`ir-tracker-custom-1.3.5.irfw`. Its signature and ESP32 image are
...
`ir-tracker-assets-1.3.5.bin`. Settings, history and the current minute
