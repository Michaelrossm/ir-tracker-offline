...
String recoveryPage() {
  ...
  html += F("</pre><a class='button' href='/api/v1/status'>Status-API</a>"
            ...
            "<input name='update' type='file' accept='.irup' required>"
            "<button>Vollständiges Update installieren</button></form></section>"
  ...
  html += F("async function send(url,body,headers={}){headers['X-CSRF-Token']=csrf;"
            ...
            "const button=e.target.querySelector('button');if(button.disabled)return;button.disabled=true;try{"
            "await send('/api/v1/update/bundle',new FormData(e.target));"
            "out.textContent='Update erfolgreich. Warte auf Neustart …';let attempts=0;"
            "const poll=async()=>{try{const r=await fetch('/api/v1/status',{cache:'no-store',signal:AbortSignal.timeout(2500)});"
            "if(r.ok){const s=await r.json();if(s.asset_manifest_valid){location.replace('/?updated='+Date.now());return}}}catch(_){}"
            "if(++attempts<60)setTimeout(poll,2000);else{out.textContent='Tracker noch nicht bereit. Bitte später neu laden.';button.disabled=false}};"
            "setTimeout(poll,5000);}catch(_){button.disabled=false}};"
