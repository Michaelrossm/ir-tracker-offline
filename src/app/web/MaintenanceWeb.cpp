...
void handleMaintenancePage() {
  ...
  body += F(
      ...
      "<div class='card'><h2>Vollständiges Update</h2>"
      "<p>Installiert die kryptografisch geprüfte Firmware und die passende Weboberfläche gemeinsam. "
      "Vor dem Schreiben wird die 64-kB-Asset-Partition gesichert; History und Einstellungen werden nicht verändert.</p>"
      "<form id='combinedUpdate'><label>Signiertes Gesamtupdate (.irup)</label><input id='combinedBundle' type='file' accept='.irup' required>"
      "<label><input id='combinedBackup' type='checkbox' required> Asset-Backup wurde heruntergeladen und aufbewahrt</label>"
      "<button type='submit'>Vollständiges WLAN-Update installieren</button></form></div>"
