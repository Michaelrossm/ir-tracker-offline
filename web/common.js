...
window.irGapEdges=(values,from,to,step,now=Math.floor(Date.now()/1000))=>{if(!values.length)return{leading:false,trailing:false};const expectedEnd=Math.min(to,now),current=to>=now-step,trailingLimit=current?Math.max(step*2.5,15):step*1.5;return{leading:values[0].ts-from>step*1.5,trailing:expectedEnd-values.at(-1).ts>trailingLimit}};
...
window.irGapRegions=(values,from,to,step,now=Math.floor(Date.now()/1000))=>{const end=Math.min(to,now);if(end<=from)return[];if(!values.length)return[{start:from,end}];const regions=[],edges=irGapEdges(values,from,to,step,now),add=(start,finish)=>{start=Math.max(from,start);finish=Math.min(end,finish);if(finish>start)regions.push({start,end:finish})};if(edges.leading)add(from,values[0].ts);values.forEach((value,index)=>{if(irGapAfter(values,index,step))add(values[index-1].ts+step,value.ts)});if(edges.trailing)add(values.at(-1).ts+step,end);return regions};
window.irRefineGapRegions=(coarse,precise,precisionFrom)=>{if(!Number.isFinite(precisionFrom))return coarse;const result=[];coarse.forEach(region=>{if(region.start>=precisionFrom)return;const end=Math.min(region.end,precisionFrom);if(end>region.start)result.push({start:region.start,end})});precise.forEach(region=>{if(Number.isFinite(region.start)&&Number.isFinite(region.end)&&region.end>region.start)result.push({start:region.start,end:region.end})});return result};
